...
api_key = "y2u%H%hpZID603j41Clkfks23k2dkglfjkfe3k4k2k4lk23qj2"
app = Flask(__name__)
...
def do_ws_srv_telnet(source):
    HOST = "localhost"
    print("telnet localhost 8078 SEL {}".format(source))
    tn = Telnet(HOST,8078)
...
def getCurrentShowConnection():
    for connection in connections:
        if (connection["startTimestamp"] <= datetime.datetime.now().timestamp()) and (connection["endTimestamp"] >= getNextHourTimestamp()):
            return connection
    return None

def getNextHourConnection():
